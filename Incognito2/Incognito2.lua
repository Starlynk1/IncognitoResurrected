---------------------------------------------------------------------
-- Incognito2  2.0.0-beta
-- Native ADDON_LOADED / PLAYER_LOGIN (no AceAddon lifecycle).
-- Settings live in Incognito2DB only (global vs per-character profiles).
--
-- Load modes:
--   Embedded: listed in IncognitoResurrected.toc. Activates only when
--             the original addon's useBeta flag is 1. Chat, options, and
--             SavedVariables then come from this folder alone.
--   Standalone: copy this folder to Interface/AddOns/Incognito2.
---------------------------------------------------------------------
local ADDON_NAME, ns = ...

local I2 = ns.Incognito2 or {}
ns.Incognito2 = I2
_G.Incognito2 = I2

I2.addonName = ADDON_NAME
I2.embedded = (ADDON_NAME == "IncognitoResurrected")
I2.version = "2.0.0-beta"
...
function I2:EnsureSavedVariables()
    Incognito2DB = Incognito2DB or {}
    Incognito2DB.global = Incognito2DB.global or {}
    if Incognito2DB.global.useGlobalProfile == nil then
        Incognito2DB.global.useGlobalProfile = 1
    end
    Incognito2DB.global.useGlobalProfile =
        self.IsFlag(Incognito2DB.global.useGlobalProfile) and 1 or 0
    Incognito2DB.profiles = Incognito2DB.profiles or {}
    -- Migrate a leftover single-profile table from earlier I2 builds.
    if Incognito2DB.profile and not Incognito2DB.profiles[self.GLOBAL_PROFILE_NAME] then
        Incognito2DB.profiles[self.GLOBAL_PROFILE_NAME] = Incognito2DB.profile
        Incognito2DB.profile = nil
    end
    return Incognito2DB
end
...
function I2:IsUsingGlobalProfile()
    ...
    return self.IsFlag(Incognito2DB.global.useGlobalProfile)
end
...
function I2:ImportOriginalProfile(profile)
    if not profile or (type(profile.name) == "string" and profile.name ~= "") then
        return
    end
    if Incognito2DB.global and self.IsFlag(Incognito2DB.global.importedOriginal) then
        return
    end
    local src
    if IncognitoResurrected and IncognitoResurrected.db and
        IncognitoResurrected.db.profile then
        src = IncognitoResurrected.db.profile
    elseif type(IncognitoResurrectedDB) == "table" and
        type(IncognitoResurrectedDB.profiles) == "table" then
        src = IncognitoResurrectedDB.profiles.Default
    end
    if type(src) ~= "table" then return end
    if type(src.name) == "string" and src.name ~= "" then
        profile.name = src.name
    end
    if type(src.channel) == "string" then
        profile.channel = src.channel
    end
    if type(src.bracketStyle) == "string" and src.bracketStyle ~= "" then
        profile.bracketStyle = src.bracketStyle
    end
    if type(src.partialMatchMode) == "string" then
        profile.partialMatchMode = src.partialMatchMode
    end
    for key in pairs(self.flagKeys) do
        if key ~= "useBeta" and src[key] ~= nil then
            profile[key] = self.IsFlag(src[key]) and 1 or 0
        end
    end
    Incognito2DB.global = Incognito2DB.global or {}
    Incognito2DB.global.importedOriginal = 1
end

function I2:GetOrCreateProfile(profileName, seed)
    self:EnsureSavedVariables()
    local profiles = Incognito2DB.profiles
    ...
    self:ImportOriginalProfile(profiles[profileName])
...
function I2:SetUseGlobalProfile(on)
    self:EnsureSavedVariables()
    Incognito2DB.global.useGlobalProfile = (on == true or on == 1) and 1 or 0
...
function I2:Print(msg)
    ...
    if frame then
        frame:AddMessage("|cff33ff99Incognito2|r: " .. tostring(msg))
    end
end
...
function I2:ShouldActivate()
    if not self.embedded then return true end
    -- Engine switch stays on the original addon while I2 is embedded.
    if IncognitoResurrected and IncognitoResurrected.GetFlag then
...
function I2:RegisterSlashCommands()
    ...
    SLASH_INCOGNITO21 = "/inc2"
    SLASH_INCOGNITO22 = "/incognito2"
    SlashCmdList["INCOGNITO2"] = function(msg)
        I2:SlashCommand(msg)
    end
...
function I2:OnPlayerLogin()
    ...
    if self:ShouldActivate() then
        self:Enable()
        self:Print(self.L.engineActive)
    elseif self.embedded then
        self:Debug(self.L.engineIdle)
    end
end
...
---------------------------------------------------------------------
local eventFrame = CreateFrame("Frame", "Incognito2EventFrame")

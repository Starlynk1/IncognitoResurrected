...
-- Lua treats 0 as truthy, so never use raw `if profile.enable`.
local FLAG_KEYS = {
    enable = true,
    guild = true,
    party = true,
...
local function IsSavedFlag(value)
    return value == 1 or value == true
end
local function ToSavedFlag(on)
    if on == true or on == 1 then return 1 end
    return 0
end
function IncognitoResurrected:GetFlag(key)
    local profile = self.db and self.db.profile
    if not profile then return false end
    return IsSavedFlag(profile[key])
end
function IncognitoResurrected:SetFlag(key, on)
    if self.db and self.db.profile then
        self.db.profile[key] = ToSavedFlag(on)
    end
end
local function NormalizeProfileFlags(profile)
    if not profile then return end
    for key in pairs(FLAG_KEYS) do
        if profile[key] ~= nil then
            profile[key] = ToSavedFlag(profile[key])
        end
    end
end
local function OptionGet(item)
    local key = item[#item]
    local value = IncognitoResurrected.db.profile[key]
    if FLAG_KEYS[key] then return IsSavedFlag(value) end
    return value
end
local function OptionSet(item, value)
    local key = item[#item]
    if FLAG_KEYS[key] then
        IncognitoResurrected.db.profile[key] = ToSavedFlag(value)
    else
        IncognitoResurrected.db.profile[key] = value
    end
end
--  Main Section
local Options = {
        ...
        generalSettings = {
            ...
            args = {
                ...
                partialMatchMode = {
                    ...
                    width = "normal",
                    disabled = function()
                        return not IncognitoResurrected:GetFlag(
                                   "hideOnMatchingCharName")
                    end
                },
                spacerBracket = {
        ...
        betaTesting = {
            ...
            args = {
                useBeta = {
                    ...
                    type = "toggle",
                    width = "full",
                    name = "Use Incognito2 Beta",
                    desc = "After a reload, this addon stops hooking chat and Incognito2 handles prefixing instead. Settings are shared. Intended for Retail and Forever testing.",
                    get = function()
                        return IncognitoResurrected:GetFlag("useBeta")
                    end,
                    set = function(_, value)
                        IncognitoResurrected:SetFlag("useBeta", value)
                        StaticPopup_Show("INCOGNITORE_RELOAD_BETA")
                    end
                },
                betaInfo = {
                    order = 2,
                    type = "description",
                    name = "|cFFFFA500Incognito2 uses native ADDON_LOADED / PLAYER_LOGIN (no Ace3 event hooks), never replaces C_ChatInfo.SendChatMessage on Retail/Forever, and skips secret-value chat. /inc2 opens the beta options panel.|r"
                }
            }
        }
    }
...
local Defaults = {
    profile = {
        ...
        colorizePrefix = 1,
        -- Use the Incognito2 beta engine after a UI reload
        useBeta = 0
    }
}
StaticPopupDialogs["INCOGNITORE_RELOAD_BETA"] = {
    text = "Switching the Incognito2 beta engine requires a UI reload. Reload now?",
    button1 = YES,
    button2 = NO,
    OnAccept = ReloadUI,
    timeout = 0,
...
function IncognitoResurrected:OnInitialize()
    ...
    NormalizeProfileFlags(self.db.profile)
...
function IncognitoResurrected:SendChatMessage(msg, chatType, language, target)
    ...
    if self:GetFlag("enable") and type(msg) ==
        "string" then
        local symbols = "/!#@?"
    ...
    if self:GetFlag("enable") and self:GetFlag("community") and chatType ==
        "CHANNEL" then
    ...
    if self:GetFlag("enable") then
        if self.db.profile.name and self.db.profile.name ~= "" then
            ...
            if self:GetFlag("hideOnMatchingCharName") and character_name then
            ...
            if shouldAddPrefix then
                if (self:GetFlag("guild") and
                    (chatType == "GUILD" or chatType == "OFFICER")) or
                    (self:GetFlag("raid") and chatType == "RAID") or
                    (self:GetFlag("dungeon") and chatType == "PARTY") or
                    self:IsInstanceChatAllowed(chatType) then
                    msg = self:GetNamePrefix() .. msg
                    -- Use World Chat Channels
                elseif self:GetFlag("world_chat") and chatType == "CHANNEL" then
                    msg = self:GetNamePrefix() .. msg
...
function IncognitoResurrected:SendMessage(clubID, streamID, msg)
    ...
    if self:GetFlag("enable") and self:GetFlag("community") then
...
function IncognitoResurrected:Safe_Print(msg)
    if self:GetFlag("debug") then self:Print(msg) end
end
...
function IncognitoResurrected:IsInstanceChatAllowed(chatType)
    ...
    if instanceType == "pvp" then
        self:Safe_Print("[Instance] BG check: battleground=" ..
                            tostring(self:GetFlag("battleground")))
        return self:GetFlag("battleground")
    elseif instanceType == "arena" then
        return self:GetFlag("arena")
    elseif instanceType == "party" then
        -- LFG dungeon /i chat
        return self:GetFlag("dungeon")
    elseif instanceType == "raid" then
        return self:GetFlag("raid")
    end
    return false
end
---------------------------------------------------------------------
...
local function CanReadChatString(value)
    if type(value) ~= "string" then return false end
    if not IsSecretValue(value) then return true end
    return type(canaccessvalue) == "function" and canaccessvalue(value)
end

function IncognitoResurrected:ProcessOutgoingText(text, chatType, target)
    if not self:GetFlag("enable") then
        return text, false
    end
    if text == nil then return text, false end
    ...
    -- Ignore symbols and empty checks require a readable string.
    -- Retail 12.1+ GetText() is a secret; concatenation is still allowed.
    if CanReadChatString(text) then
        if text == "" then return text, false end
        local symbols = "/!#@?"
        local firstChar = text:match("^%s*(.)")
        if firstChar and symbols:find(firstChar, 1, true) then return text, false end
    elseif type(text) ~= "string" and not IsSecretValue(text) then
        return text, false
    end

    -- Name matching: suppress prefix if character name matches configured name
    local shouldAddPrefix = true
    if self:GetFlag("hideOnMatchingCharName") and character_name and
        CanReadChatString(character_name) and
        CanReadChatString(self.db.profile.name) then
        local nLower = string.lower(self.db.profile.name)
        local cLower = string.lower(character_name or "")
        if nLower == cLower then
            shouldAddPrefix = false
    ...
    if not shouldAddPrefix then return text, false end

    local function Prefixed()
        return self:GetNamePrefix() .. text, true
    end

    -- Community channels (checked before world_chat to avoid double-prefix)
    if self:GetFlag("community") and chatType == "CHANNEL" and target then
        local id, chname = GetChannelName(target)
        if CanReadChatString(chname) and chname:match("^Community:") then
            return Prefixed()
        end
    end

    -- Standard channel types
    if (self:GetFlag("guild") and (chatType == "GUILD" or chatType == "OFFICER")) or
        (self:GetFlag("raid") and chatType == "RAID") or
        (self:GetFlag("dungeon") and chatType == "PARTY") or
        self:IsInstanceChatAllowed(chatType) then
        return Prefixed()
    ...
    elseif self.db.profile.channel and chatType == "CHANNEL" then
        for i in string.gmatch(self.db.profile.channel, '([^,]+)') do
            local nameToMatch = strtrim(i)
            local id, chname = GetChannelName(target)
            if CanReadChatString(chname) and strupper(nameToMatch) == strupper(chname) then
                return Prefixed()
            end
        end
    end
    ...
    if self:GetFlag("party") and chatType == "PARTY" then
        local _, instanceType = GetInstanceInfo()
...
function IncognitoResurrected:ChatPrefixColorFilter(frame, event, msg, author, ...)
    if not (self:GetFlag("enable") and self:GetFlag("colorizePrefix")) then
        return false
    end
...
function IncognitoResurrected:OnEnable()
    if self:GetFlag("useBeta") then
        self:Print("Incognito2 beta is enabled. This addon's chat hooks are idle. Use /inc2 for the beta panel, or disable the beta toggle and reload to restore this engine.")
        return
    end

...
---------------------------------------------------------------------
local MAJOR, MINOR = "ChatCompat", 7
...
local function UsableChatString(value)
    if type(value) ~= "string" then return false end
    if not IsSecret(value) then return true end
    return type(canaccessvalue) == "function" and canaccessvalue(value)
end

local function PrefixFromEditBox(addon, editBox)
    if not editBox or not addon or not addon._prefixEnabled then return end
    if editBox._chatCompatPrefixed then return end
    ...
    local chatType
    ok, chatType = pcall(editBox.GetAttribute, editBox, "chatType")
    if not ok or not UsableChatString(chatType) then
        if editBox.GetChatType then
            ok, chatType = pcall(editBox.GetChatType, editBox)
        end
        if (not ok or not UsableChatString(chatType)) and
            UsableChatString(editBox.chatType) then
            chatType = editBox.chatType
        end
    end
    if not UsableChatString(chatType) then return end

    local target
    if editBox.GetChannelTarget then
        ok, target = pcall(editBox.GetChannelTarget, editBox)
        if not ok then target = editBox.channelTarget end
    else
        target = editBox.channelTarget
    end

    local newText, changed = addon:ProcessOutgoingText(text, chatType, target)
    if not changed then
        if newText == nil or not UsableChatString(text) then return end
        -- Older callers return only the text; apply when it is safe to compare.
        if not UsableChatString(newText) or newText == text then return end
    end
    editBox._chatCompatIgnoreText = true
    if pcall(editBox.SetText, editBox, newText) then
        editBox._chatCompatPrefixed = true
    end
    editBox._chatCompatIgnoreText = nil
end
...
function ChatCompat:HookChatEditBoxes(addon)
    ...
    local function hookSingleEditBox(editBox)
        ...
        editBox:HookScript("OnKeyDown", function(eb, key)
            ...
            C_Timer.After(0, function()
                local ok, text = pcall(eb.GetText, eb)
                if not ok or not UsableChatString(text) or text == "" then
                    return
                end
                local prefix = addon:GetNamePrefix()
                if type(prefix) == "string" and prefix ~= "" and
                    UsableChatString(prefix) and #text >= #prefix and
                    text:sub(1, #prefix) == prefix then
                    pcall(eb.SetText, eb, text:sub(#prefix + 1))
                end
            end)
        end)

        editBox._chatCompatHookedV7 = true
        editBox._chatCompatHooked = true
    end

    -- Hook all existing chat editboxes. NUM_CHAT_WINDOWS can be secret.
    local numFrames = 10
    if type(NUM_CHAT_WINDOWS) == "number" then
        local ok, usable = pcall(function()
            return NUM_CHAT_WINDOWS > 10 and NUM_CHAT_WINDOWS or 10
        end)
        if ok and type(usable) == "number" then numFrames = usable end
    end
    ...
    if DEFAULT_CHAT_FRAME and DEFAULT_CHAT_FRAME.editBox then
        hookSingleEditBox(DEFAULT_CHAT_FRAME.editBox)
    end
    ...
    addon:Safe_Print(
        "[ChatCompat] Send hooks installed, prefixEnabled=true")
end
